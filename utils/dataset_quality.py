...
def main():
    ...
    submits_root_dir = os.path.dirname(os.path.abspath(submits_dir))

    fold = int(os.path.basename(submits_root_dir).split('-')[-3])

    print(f'fold={fold}')

    scores = []

    for eopatch_idx in get_patch_indices(submits_dir):

        print(f'eopatch-{eopatch_idx:04d}')

        image_paths = sorted(glob.glob(os.path.join(submits_dir, f'*_eopatch-{eopatch_idx:04d}-{eopatch_idx:04d}_*.jpg')))
    ...
    if not os.path.exists(os.path.join(submits_root_dir, 'ssim_scores.pkl')):
        df = pd.DataFrame(list(zip(folds, ssim_scores)), columns=['Fold', 'SSIM'])
        df.to_pickle(os.path.join(submits_root_dir, 'ssim_scores.pkl'))
